    ...
    def setup(self, device):
        ...
        if model_source in ["modelscope", "auto"]:
            try:
                ...
                logger.info(f"📦 Model download source: ModelScope (国内推荐)")
            except ImportError:
...
def start_litserve_workers(
    ...
    # CUDA_VISIBLE_DEVICES 或 --devices 传入的字符串（如 "0"、"0,1"）必须转为 list[int]：
    #   - litserve 语义：int 表示"设备数量"（devices=0 → range(0) → 空，会崩溃）
    #   - list 表示"设备索引列表"（devices=[0] → 1 个设备，正确）
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Enable PaddleOCR VL VLLM engine (default: False)",
    )
    parser.add_argument(
        "--paddleocr-vl-vllm-api-list",
        type=parse_list_arg,
        default=[],
        help='PaddleOCR VL VLLM API list (Python list literal format, e.g.: \'["http://127.0.0.1:8000/v1", "http://127.0.0.1:28657/v1"]\')',
    )
    parser.add_argument(
        "--mineru-vllm-api-list",
        type=parse_list_arg,
        default=[],
        help='MinerU VLM vLLM API list (Python list literal format, e.g.: \'["http://127.0.0.1:8000/v1"]\')',
    )
