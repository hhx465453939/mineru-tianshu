...
import { downloadTasksArchive, batchCancelTasks, batchDeleteTasks, batchRestartTasks } from '@/api/taskApi'
...
export const useTaskStore = defineStore('task', () => {
  ...
  const selectedCancellableIds = computed(() =>
    tasks.value
      .filter(t => selectedTaskIds.value.includes(t.task_id) && (t.status === 'pending' || t.status === 'processing'))
      .map(t => t.task_id)
  )
   ...
   * 批量取消/停止任务
   */
  async function storeBatchCancelTasks() {
    const ids = selectedCancellableIds.value
    if (ids.length === 0) return
    try {
      const res = await batchCancelTasks(ids)
      // 更新本地状态
      ids.forEach(id => updateLocalTaskStatus(id, 'cancelled' as TaskStatus))
      selectedTaskIds.value = []
      return res
    } catch (err: any) {
      error.value = err.message || '批量取消失败'
      throw err
    }
  }
   ...
   * 批量硬删除任务
   */
  async function storeBatchDeleteTasks() {
   ...
   * 批量重启任务
   */
  async function storeBatchRestartTasks() {
    const ids = [...selectedTaskIds.value]
    if (ids.length === 0) return
    try {
      const res = await batchRestartTasks(ids)
      // 更新本地状态为 pending
      ids.forEach(id => updateLocalTaskStatus(id, 'pending' as TaskStatus))
      selectedTaskIds.value = []
      return res
    } catch (err: any) {
      error.value = err.message || '批量重启失败'
      throw err
    }
  }

  /**
   * 批量下载归档 - 返回 blob 和文件名供调用方处理
   */
  async function storeDownloadTasksArchive(taskIds?: string[]) {
    const ids = taskIds || [...selectedTaskIds.value]
    if (ids.length === 0)
      throw new Error('No tasks selected')
    try {
      return await downloadTasksArchive(ids)
    } catch (err: any) {
  ...
  return {
    ...
    clearFailedTasks,
    selectedTaskIds,
    selectedCancellableIds,
    hasSelection,
    storeBatchCancelTasks,
    storeBatchDeleteTasks,
    storeBatchRestartTasks,
    storeDownloadTasksArchive,
    toggleTaskSelection,
    selectAllTasks,
    clearSelection,
    pollTaskStatus,
