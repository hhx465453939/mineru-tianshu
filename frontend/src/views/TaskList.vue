      ...
      <div v-if="selectedTasks.length > 0" class="bg-primary-50/50 px-6 py-3 border-b border-primary-100 flex items-center justify-between transition-all animate-fade-in">
        ...
        <div class="flex gap-2">
          ...
          <button
            ...
            :disabled="actionLoading"
            class="bg-white border border-red-300 text-red-700 hover:bg-red-100 px-3 py-1.5 rounded-md text-xs font-medium transition-colors flex items-center shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
          >
          ...
          <button
            ...
            :disabled="actionLoading"
            class="bg-white border border-blue-200 text-blue-600 hover:bg-blue-50 hover:text-blue-700 px-3 py-1.5 rounded-md text-xs font-medium transition-colors flex items-center shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
          >
          ...
          <button
            ...
            class="bg-white border border-green-200 text-green-600 hover:bg-green-50 hover:text-green-700 px-3 py-1.5 rounded-md text-xs font-medium transition-colors flex items-center shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <Download class="w-3.5 h-3.5 mr-1.5" />
            {{ downloading ? $t('common.downloading') : $t('task.batchDownloadArchive') }}
          </button>
          <button
...
import {
  ...
  XCircle, Copy, Trash2, Play, Pause, RotateCw, Eraser, List, Clock, ChevronDown, Download
...
const selectableTaskIdsOnPage = computed(() =>
  tasks.value.map(task => task.task_id)
)
...
// ============================================================
// 批量操作增强：批量删除 / 重启 / 下载（local features）
// ============================================================

/** 批量删除确认 */
function askBatchDelete() {
  confirmDialogTitle.value = t('task.batchDelete')
  confirmDialogMessage.value = t('task.confirmBatchDelete', { count: selectedTasks.value.length })
  confirmDialogType.value = 'danger'
  showConfirmDialog.value = true
  pendingAction = async () => {
    actionLoading.value = true
    ...
    try {
      await taskStore.storeBatchDeleteTasks()
      selectedTasks.value = []
      selectAll.value = false
      await refreshTasks(true)
    } finally {
...
function askBatchRestart() {
  confirmDialogTitle.value = t('task.batchRestart')
  confirmDialogMessage.value = t('task.confirmBatchRestart', { count: selectedFailedIds.value.length })
  confirmDialogType.value = 'info'
  showConfirmDialog.value = true
  pendingAction = async () => {
    actionLoading.value = true
    currentActionType.value = 'batchRestart'
    try {
      await taskStore.storeBatchRestartTasks()
      selectedTasks.value = []
      selectAll.value = false
      await refreshTasks(true)
    } finally {
...
async function batchDownloadResults() {
  ...
  try {
    const { blob, fileName } = await taskStore.storeDownloadTasksArchive(selectedCompletedTasks.value)
